...
def forward_backward_prop(data, labels, params, dimensions):
    ...
    cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=y_hat_beforesoftmax))
    ...
    with tf.Session() as sess:
        ...
        for i in range(200000):
            ...
            if i%100 == 0:
                print('after %s ,loss='%i,sess.run(cost,feed_dict={x:np.mat(data),y:np.mat(labels)}))
